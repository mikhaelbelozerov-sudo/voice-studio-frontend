import { Gift, Globe, MessageCircle, Moon, Sun, UserRound } from "lucide-react";
...
import { getTimeOfDayGreetingKey } from "../utils/timeOfDayGreeting";
...
export const ProfilePage = () => {
  ...
  const displayName = user?.first_name || t("common.guest");
  const greetingKey = useMemo(() => getTimeOfDayGreetingKey(), []);
  ...
  return (
    <AppPage>
      ...
      {isLoading ? (
        <div className="flex items-center gap-2 rounded-2xl border border-slate-100 bg-white p-4 text-sm text-slate-600 shadow-sm dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
          <Spinner />
      ...
      <div className="space-y-4 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div>
          <div className="flex items-center gap-2">
            <UserRound className="h-4 w-4 shrink-0 text-blue-600 dark:text-blue-400" aria-hidden />
            <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">
              {t(greetingKey, { name: displayName })}
            </h2>
          </div>
          <p className="mt-1 text-xs leading-relaxed text-slate-500 dark:text-slate-400">{t("profile.greetingSubtitle")}</p>
        </div>
      </div>
      ...
      <div className="space-y-4 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div>
          <div className="flex items-center gap-2">
            <Globe className="h-4 w-4 shrink-0 text-blue-600 dark:text-blue-400" aria-hidden />
            <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">{t("profile.language")}</h2>
          </div>
          <p className="mt-1 text-xs leading-relaxed text-slate-500 dark:text-slate-400">{t("profile.languageDescription")}</p>
        </div>
        <DropdownMenu
      ...
      <div className="space-y-4 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div>
          <div className="flex items-center gap-2">
            {theme === "dark" ? (
              <Moon className="h-4 w-4 shrink-0 text-blue-600 dark:text-blue-400" aria-hidden />
            ) : (
              <Sun className="h-4 w-4 shrink-0 text-blue-600 dark:text-blue-400" aria-hidden />
            )}
            <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">{t("profile.theme")}</h2>
          </div>
        </div>
      ...
      <div className="space-y-4 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">{t("profile.inviteTitle")}</h2>
          </div>
          <p className="mt-1 text-xs leading-relaxed text-slate-500 dark:text-slate-400">{t("profile.inviteSubtitle")}</p>
        </div>
        {profile?.referral ? (
          <div className="space-y-2 rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs leading-relaxed text-slate-600 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300">
            {profile.referral.referralAtMonthlyLimit ? (
            ...
            {profile.referral.referralPendingCount > 0 ? (
              <p>{t("profile.referralPendingLine", { count: profile.referral.referralPendingCount })}</p>
            ) : null}
        ...
        {!inviteUrl ? (
          <p className="text-xs text-amber-700 dark:text-amber-300">{t("profile.inviteLinkMissing")}</p>
        ) : (
          <div className="space-y-2">
            <p className="break-all rounded-xl border border-slate-200 bg-slate-50 p-2.5 text-xs text-slate-600 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-300">
              {inviteUrl}
            </p>
      ...
      <div className="space-y-4 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div>
          <div className="flex items-center gap-2">
            <MessageCircle className="h-4 w-4 shrink-0 text-blue-600 dark:text-blue-400" aria-hidden />
            <h2 className="text-base font-semibold text-slate-900 dark:text-slate-100">{t("profile.supportTitle")}</h2>
          </div>
          <p className="mt-1 text-xs leading-relaxed text-slate-500 dark:text-slate-400">{t("profile.supportSubtitle")}</p>
        </div>
        <Button className="w-full" onClick={handleOpenSupport} disabled={!SUPPORT_LINK}>
